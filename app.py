...
def detect_plate(img):
    ...
    return data(color, text_extraction)
...
def upload_file():
    if request.method == 'POST':
        ...
        if allowed_image(image.filename):
            ...
            plateimg = cv2.imread(os.path.join(app.config["IMAGE_UPLOADS"], filename))
            # bars = []
            # cv2.imshow('Dominant colors', img_bar)
            flash('File upload Successfully !', "success")
            palte_info = detect_plate(plateimg)
